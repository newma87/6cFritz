...
class Protocol (object):
	...
	def packCommand(self, command, byteArray = []):
		if command >= 128:
			print "[Protocol]packCommand: command must less than 128"
			return None
		buf = [chr(command | 128)]
		dLen = len(byteArray)
		# add length
	...
	def packShortPinValue(self, command, pin, val):
		self.packCommand(command, [(pin & 127), (val & 127), ((val >> 7) & 127)])

	def packIntPinValue(self, command, pin, val):
		self.packCommand(command, [(pin & 127), (val & 127), ((val >> 7) & 127), ((val >> 14) & 127), ((val >> 21) & 127), ((val >> 28) & 127)])

	def packBytePinValue(self, command, pin, val):
		self.packCommand(command, [(pin & 127), (val & 127)])
	...
	def packShortArrayCommand(self, command, shortArray = []):
		...
		self.packCommand(command, byteArray)
	...
	def unpackByteArray2ShortArray(self, byteArray):
		...
		while (i < length):
			...
			if length - i == 1:
				print "warnning[Protocol]unpackByteArray2ShortArray: argument byteArray is odd"
				break;
			print '[', ord(byteArray[i]), ', ' , ord(byteArray[i + 1]), ']'
			shortArray.append((ord(byteArray[i]) & 127) | ((ord(byteArray[i + 1]) & 127) << 7))
			i += 2
		return shortArray
