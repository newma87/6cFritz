        ...
        */
        ...
        public bool start()
        {
            if (!connect("localhost"))
            {
                ...
                // if you don't want RR to appear (which is recommended in case the camera is not on) add in -faceless
                //if (StartInstance(roboPath + "\\RoboRealm.exe", "-faceless -api_port " + SERVER_PORTNUM, SERVER_PORTNUM))
                if (StartInstance(roboPath + "\\RoboRealm.exe", "-api_port " + SERVER_PORTNUM, SERVER_PORTNUM))
                {
                    if (!connect("localhost"))
                    {
