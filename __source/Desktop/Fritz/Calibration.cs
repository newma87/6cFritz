...
namespace Fritz
{
    public partial class Calibration : Form
    {
        ...
        private void timer1_Tick_1(object sender, EventArgs e)
        {
            ...
            {
                if (enableIR.Checked)
                    irDistance.Text = Convert.ToString(conductor.GetRawIRValue((int)indexToPin[irPin.SelectedIndex]));

                if (enableSonar.Checked)
                    sonarDistance.Text = Convert.ToString(conductor.GetRawSonarValue((int)indexToPin[sonarTriggerPin.SelectedIndex], (int)indexToPin[sonarEchoPin.SelectedIndex]));
            }
        }
