...
namespace Fritz
{
    public partial class Form1 : Form
    {
        ...
        RoboRealm.RR_API rr = null;

        bool calibrating = false;

        Face_Detect_And_Greet fdagDlg = null;
...
*/
        ...
        private void toolStrip_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            if (e.ClickedItem.Name.Equals("uploadStripButton"))
            {
              ExecuteUpload();
            }
            else
        ...
        public void recorder_PlaybackStoppedCallback(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                uploadStripButton.Enabled = true;
        ...
        private void ExecutePlayback(bool repeat)
        {
            ...
            uploadStripButton.Enabled = false;
        ...
        private void faceDetectAndGreetToolStripMenuItem_Click(object sender, EventArgs e)
        {
          ...
          if (fdagDlg==null)
            fdagDlg = new Face_Detect_And_Greet(rr, conductor);
        ...
        {
            SequenceRecorder dlg = new SequenceRecorder(conductor, keyboard, joy);
            dlg.ShowDialog();
            if (dlg.DialogResult == DialogResult.OK)
            {
        ...
        {
          int len = conductor.Upload();
          if (len==0)
            MessageBox.Show("Failed to upload Sequence to Arduino", "Connection Error", MessageBoxButtons.OK);
          else
            if (len<0)
              MessageBox.Show("Uploaded " + Conductor.MAX_SEQ_BYTES + " bytes but sequence exceeded capacity by " + (-len) + " bytes", "Upload Partially Successful", MessageBoxButtons.OK);
            else
              MessageBox.Show("Uploaded " + len + " Sequence bytes to Arduino with " + (Conductor.MAX_SEQ_BYTES - len) + " bytes left", "Upload Successful", MessageBoxButtons.OK);
        }
